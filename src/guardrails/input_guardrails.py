...
def detect_injection(user_input: str) -> bool:
    ...
    INJECTION_PATTERNS = [
        ...
        r"you are now\b",
        r"(reveal|show|print|output|display|give me) (your )?(system prompt|instructions|config|configuration|credentials)",
        ...
        r"(tiết lộ|tiet lo|cho tôi xem|cho toi xem).{0,20}(mật khẩu|mat khau|system prompt|hướng dẫn)",
    ]
...
def topic_filter(user_input: str) -> bool:
    ...
    # Block if contains any dangerous topic
    for topic in BLOCKED_TOPICS:
        if topic in input_lower:
            return True

    # Block if no allowed banking topic found
        ...
        *,
        ...
        if detect_injection(text):
            ...
            return self._block_response(
                ...
                "Tôi chỉ có thể hỗ trợ các câu hỏi liên quan đến dịch vụ ngân hàng."
            )

        if topic_filter(text):
            self.blocked_count += 1
            return self._block_response(
                "Tôi là trợ lý VinBank và chỉ hỗ trợ các vấn đề liên quan đến tài khoản, "
                "giao dịch, và dịch vụ ngân hàng. Bạn cần hỗ trợ gì về ngân hàng không?"
            )
